// https://github.com/cinnyapp/cinny/blob/a1a822c5b64eb4e31edbf9cbd4bd12f3926f084b/src/app/utils/matrix-uia.ts
import { AuthType, type IAuthData, type UIAFlow } from 'matrix-js-sdk';

export const getSupportedUIAFlows = (
  uiaFlows: UIAFlow[],
...
export const getUIAErrorCode = (authData: IAuthData): string | undefined => {
  const errorCode =
    'errcode' in authData && typeof authData.errcode === 'string'
      ? authData.errcode
      : undefined;
...
export const getUIAError = (authData: IAuthData): string | undefined => {
  const errorCode =
    'error' in authData && typeof authData.error === 'string'
      ? authData.error
      : undefined;
...
export const getUIAFlowForStages = (
  uiaFlows: UIAFlow[],
  stages: string[]
): UIAFlow | undefined => {
...
export const getLoginTermUrl = (params: UIAParams): string | undefined => {
  ...
  if (terms && 'policies' in terms && typeof terms.policies === 'object') {
    ...
    if (
      'privacy_policy' in terms.policies &&
      typeof terms.policies.privacy_policy === 'object'
    ) {
      ...
      const url = langToPolicy.en?.url as string | undefined;
      if (typeof url === 'string') return url;

      const firstKey = Object.keys(langToPolicy)[0];
      return langToPolicy[firstKey]?.url as string | undefined;
    }
  }
